...
/**
 * LlmClient — appel HTTP (cURL) vers une API de chat compatible OpenAI.
 *
 * Volontairement agnostique du fournisseur : OpenAI, Mistral ou tout endpoint
 * « chat-completions » compatible (même protocole : Authorization Bearer,
 * messages system/user, response_format json_object). On change de fournisseur
 * en changeant l'endpoint, le modèle et la clé dans la config (.env), pas le code.
 *
 * Gère timeout, retries avec backoff et force une sortie JSON stricte. Ne connaît
 * RIEN du métier : il transporte un couple (system, user) et renvoie le texte brut.
 */
class LlmClient
{
    /**
     * @param array<string,mixed> $config Sous-tableau 'ai' (endpoint, model, api_key, timeout, max_retries, retry_delay, temperature)
    ...
    public function chat(string $system, string $user): string
    {
        $payload = [
            ...
            // Force une réponse JSON (barrière anti texte libre) — supporté par OpenAI et Mistral.
