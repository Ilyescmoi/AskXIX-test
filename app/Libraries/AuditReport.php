...
class AuditReport
{
     ...
     */
    public function rows(array $rows, array $labels, array $quality, ?RunTrace $trace = null, ?array $grounding = null, ?array $images = null): array
    {
        ...
        foreach ($rows as $r) {
            ...
            $provA = $prov['ancrage'] ?? null;
            $ancrageKey = $grounding[$id]['ancrage'] ?? '';
            $imgMissing = array_map(static fn($m) => $m['fichier'], $images[$id]['missing'] ?? []);
            ...
            $out[] = [
                ...
                'brut_a'          => $provA['brut'] ?? '',
                'images_citees'   => count($images[$id]['cited'] ?? []),
                'images_absentes' => $imgMissing,
            ];
        }
        return $out;
    }
    ...
    public function writeCsv(array $auditRows, string $path): string
    {
        ...
        fputcsv($fh, [
            ...
            'ancrage_fidelite', 'lot_ancrage', 'statut_ia_ancrage', 'brut_ia_ancrage',
            'images_citees', 'images_absentes',
        ], ';', '"', '');
        ...
        foreach ($auditRows as $a) {
            fputcsv($fh, [
                ...
                $a['ancrage'], $a['lot_a'] ?? '', $a['statut_a'], $this->csvSafe($this->flatten($a['brut_a'])),
                $a['images_citees'], $this->csvSafe($this->flatten(implode(', ', $a['images_absentes']))),
            ], ';', '"', '');
        }
        fclose($fh);
        return $path;
    ...
    public function buildPdfData(
        ...
        return [
            ...
            'dashboard'      => $this->proofDashboard($stats, $reportData, $trace, $sourceMeta, $total, count($hallucinations)),
            ...
            // --- Fidélité à la base de connaissance (présent seulement si une base est fournie) ---
            'base_enabled'        => (bool) ($reportData['base_enabled'] ?? false),
            'base'                => [
                'files'              => $base['files'] ?? [],
                'sha256'             => $base['sha256'] ?? '',
                'docs'               => $base['doc_count'] ?? 0,
                'images'             => $base['image_count'] ?? 0,
                'bytes'              => $base['bytes'] ?? 0,
                'truncated'          => $base['truncated'] ?? false,
                'context_full_chars' => $base['context_full_chars'] ?? 0,
            ],
            'grounding'           => $reportData['grounding'] ?? [],
            'grounding_unfounded' => $reportData['grounding_unfounded'] ?? [],
            'broken_images'       => $reportData['broken_images'] ?? ['count' => 0, 'items' => []],
            'csv_name'        => $csvName,
        ];
    }
    ...
    /**
     * Tableau de bord de la preuve (page d'ouverture du document) : tout ce qu'il
     * faut pour juger la fiabilité d'un coup d'œil — source scellée, statut IA, et
     * pour chaque chiffre clé : sa valeur, sa nature, où le vérifier, ses sources.
     *
     * @return array<string,mixed>
     */
    private function proofDashboard(array $stats, array $reportData, ?RunTrace $trace, array $sourceMeta, int $total, int $hallucinationCount): array
    {
        $aiEnabled = (bool) ($reportData['ai_enabled'] ?? false);
        $sum = $trace?->summary() ?? [];

        // Sujet métier n°1 (hors catégories génériques) — même règle que le rapport.
        $topTopic = null;
        foreach ($reportData['topics'] ?? [] as $t) {
        ...
        // Chiffres clés du rapport, avec section de vérification et nb de sources.
        $claims = [
            ['label' => 'Messages analysés', 'value' => (string) $total, 'nature' => 'fact',
             'verif' => '§2 · §9', 'sources' => 'tous'],
            ['label' => 'Visiteurs uniques', 'value' => (string) $stats['unique_users'], 'nature' => 'fact',
             'verif' => '§3', 'sources' => 'registre §9'],
            ['label' => 'Taux de non-réponse', 'value' => $stats['non_response']['rate'] . ' %', 'nature' => 'fact',
             'verif' => '§5', 'sources' => count($stats['non_response']['ids']) . ' réf.'],
        ];
        if ($aiEnabled) {
            if ($topTopic !== null) {
                $claims[] = ['label' => 'Sujet métier n°1', 'value' => $topTopic['name'] . ' · ' . $topTopic['percent'] . ' %',
                             'nature' => 'estim', 'verif' => '§4', 'sources' => count($topTopic['ids']) . ' réf.'];
            }
            $claims[] = ['label' => 'Hallucinations estimées', 'value' => (string) $hallucinationCount, 'nature' => 'estim',
                         'verif' => '§6', 'sources' => $hallucinationCount > 0 ? $hallucinationCount . ' réf.' : 'aucune'];
        }
        // Fidélité à la base (si une base a été comparée).
        if (!empty($reportData['base_enabled'])) {
            $fondee = null;
            foreach ($reportData['grounding'] ?? [] as $g) {
                if ($g['key'] === 'fondee') {
                    $fondee = $g;
                    break;
                }
            }
            $broken = (int) ($reportData['broken_images']['count'] ?? 0);
            if ($aiEnabled) {
                $claims[] = ['label' => 'Réponses fondées (base)',
                             'value' => ($fondee['percent'] ?? 0) . ' % (' . ($fondee['count'] ?? 0) . ')',
                             'nature' => 'estim', 'verif' => '§6bis',
                             'sources' => $fondee ? count($fondee['ids']) . ' réf.' : '—'];
            }
            $claims[] = ['label' => 'Images inexistantes', 'value' => (string) $broken, 'nature' => 'fact',
                         'verif' => '§6bis', 'sources' => $broken > 0 ? $broken . ' réf.' : 'aucune'];
        }

        return [
            'source' => [
                'file'    => (string) ($sourceMeta['file'] ?? 'n/d'),
                'sha_ok'  => ($sourceMeta['sha256'] ?? '') !== '',
                'sha'     => substr((string) ($sourceMeta['sha256'] ?? ''), 0, 16),
                'bytes'   => (int) ($sourceMeta['bytes'] ?? 0),
                'messages' => $total,
                'period'  => ($stats['period']['start'] ?? 'n/d') . ' → ' . ($stats['period']['end'] ?? 'n/d'),
            ],
            'ai' => [
                'enabled'   => $aiEnabled,
                'model'     => (string) $this->config['ai']['model'],
                'batches'   => (int) ($sum['batches'] ?? 0),
                'ok'        => (int) ($sum['batches_ok'] ?? 0),
                'ko'        => (int) ($sum['batches_ko'] ?? 0),
                'corrected' => (int) ($sum['corrected'] ?? 0),
    ...
    private function codesLegend(): array
    {
        ...
        $legend['statut'] = [
            ...
            'COR' => 'Valeur brute hors liste, corrigée en « Non classé » (§7.3)',
            'ABS' => 'Message absent de la réponse IA → « Non classé » (§7.2)',
            'ECH' => 'Lot IA en échec → « Non classé » (§7.2)',
