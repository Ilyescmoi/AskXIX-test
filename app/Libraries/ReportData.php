...
class ReportData
{
    ...
    public function build(
        ...
        // --- Fidélité à la base de connaissance (si une base est fournie) ---
        // Comptages PHP : l'IA n'a fait qu'étiqueter chaque réponse contre la base.
        $baseEnabled = $base !== null && ((($base['context'] ?? '') !== '') || (($base['images'] ?? []) !== []));
        $groundingAgg = [];
        $groundingUnfounded = [];
        $brokenImages = ['count' => 0, 'items' => [], 'ids' => []];
        if ($baseEnabled) {
            if ($grounding !== null) {
                $groundingAgg = $this->aggregate($grounding, 'ancrage', 'grounding', $this->config['grounding_labels'], $total);
                $byId = $this->indexById($rows);
                foreach ($grounding as $id => $g) {
                    if (($g['ancrage'] ?? '') === 'non_fondee' && isset($byId[$id])) {
                        $groundingUnfounded[] = [
                            'id'       => (int) $id,
                ...
                $groundingUnfounded = array_slice($groundingUnfounded, 0, (int) $this->config['report']['top_n']);
            }
            if ($images !== null) {
                foreach ($images as $id => $im) {
                    foreach (($im['missing'] ?? []) as $miss) {
                        $brokenImages['items'][] = ['id' => (int) $id, 'url' => $miss['url'], 'fichier' => $miss['fichier']];
                        $brokenImages['ids'][] = (int) $id;
                    }
                }
                $brokenImages['ids']   = array_values(array_unique($brokenImages['ids']));
                $brokenImages['count'] = count($brokenImages['items']);
            }
            // Points « En bref » dédiés à la fidélité.
            $fondee = $this->findRow($groundingAgg, 'fondee');
            if ($fondee !== null) {
                $type = $fondee['percent'] >= 80 ? 'good' : ($fondee['percent'] >= 50 ? 'warning' : 'risk');
                $highlights[] = ['type' => $type, 'label' => 'Fidélité à la base',
                    'text' => "{$fondee['percent']} % des réponses sont étayées par la base ({$fondee['count']} sur {$total}).",
                    'ids' => $fondee['ids'], 'annexe' => '§6bis'];
            }
            $nf = $this->findRow($groundingAgg, 'non_fondee');
            if ($nf !== null && $nf['count'] > 0) {
                $highlights[] = ['type' => 'risk', 'label' => 'Réponses non fondées',
                    'text' => "{$nf['count']} réponse(s) affirment des faits absents de la base (estimé).",
                    'ids' => $nf['ids'], 'annexe' => '§6bis'];
            }
            if ($brokenImages['count'] > 0) {
                $highlights[] = ['type' => 'risk', 'label' => 'Images inexistantes',
                    'text' => "{$brokenImages['count']} image(s) citée(s) n'existent pas dans la base.",
                    'ids' => $brokenImages['ids'], 'annexe' => '§6bis'];
            }
        }
        ...
        return [
            ...
            // --- Fidélité à la base (clés présentes seulement si une base est fournie) ---
            'base_enabled'        => $baseEnabled,
            'grounding'           => $groundingAgg,
            'grounding_unfounded' => $groundingUnfounded,
